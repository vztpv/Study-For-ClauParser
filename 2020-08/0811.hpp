...
namespace clau_parser {
	...
	private:
		...
		static void ScanningNew(char* text, const size_t length, const int thr_num,
			int64_t*& _token_arr, size_t& _token_arr_size, size_t* _token_arr_start, bool use_simd)
		{
			...
			int64_t* tokens = new int64_t[2 * (length + 1)];
			...
			int64_t* tokens2 = tokens + length + 1;
		...
		static std::pair<bool, int> Scan(FILE* inFile, int thr_num,
			char*& _buffer, size_t* _buffer_len, int64_t*& _token_arr, size_t* _token_arr_len, size_t* token_arr_start, bool use_simd)
		{
			if (inFile == nullptr) {
				return { false, 0 };
			}
...

			{
...

				{
					...
					if (thr_num == 1) {
						...
						ScanningNew(buffer, file_length, thr_num, token_arr, token_arr_size, token_arr_start, use_simd);
					}
	...
	public:
		bool operator() (int thr_num, char*& buffer, size_t* buffer_len, int64_t*& token_arr, size_t* token_arr_len, size_t* token_arr_start)
		{
			bool x = Scan(pInFile, thr_num, buffer, buffer_len, token_arr, token_arr_len, token_arr_start, use_simd).second > 0;
	...
	private:
		static bool __LoadData(const char* buffer, const int64_t* token_arr, int* itemTypeCount, int64_t token_arr_len, UserType* _global,
			...
			for (int64_t i = 0; i < token_arr_len; ++i) {
				...
				case 0:
				{
					...
					if (len == 1 && (-1 != Utility::Equal(TYPE_LEFT, Utility::GetType(token_arr[i])))) {
						...
						nestedUT[braceNum]->ReserveIList(itemTypeCount[i]);
						nestedUT[braceNum]->ReserveItemList(itemTypeCount[i]);
		...
		static void __LoadData2(int64_t* token_arr, int* itemTypeCount, size_t token_arr_start, size_t start, size_t token_arr_len,
			std::vector<int>* _stack, std::vector<int>* _temp)
		{
			std::vector<int> temp;

			for (size_t i = 0; i < token_arr_len; ++i) {
				switch (Utility::GetType(token_arr[token_arr_start + start + i])) {
				case TYPE_LEFT:
					_stack->push_back(start + i);
					break;
				case TYPE_RIGHT:
					if (_stack->size() > 1) {
						_stack->pop_back();
				...
				case TYPE_ASSIGN:
					if (_stack->back() >= 0) {
						itemTypeCount[_stack->back()]--;
					}
					else if (_stack->back() <= -2) {
						temp.back()--;
					}
					break;
				default:
		...
		static bool _LoadData(InFileReserver& reserver, UserType& global, const int lex_thr_num, const int parse_num) // first, strVec.empty() must be true!!
		{
			...
			size_t token_arr_start = 0;
			{

				auto a = std::chrono::steady_clock::now();

				bool success = reserver(lex_thr_num, buffer, &buffer_total_len, token_arr, &token_arr_len, &token_arr_start);
			...
			int* itemTypeCount = (int*)calloc(token_arr_len, sizeof(int));

			{int a = clock();
				std::vector<std::thread> thr(parse_num);
				std::vector<std::vector<int>> _stack(parse_num);
				std::vector<std::vector<int>> _temp(parse_num);

				std::vector<size_t> len(parse_num, 0);

				for (size_t i = 0; i < parse_num - 1; ++i) {
					len[i] = (token_arr_len / parse_num * (i + 1)) - (token_arr_len / parse_num * i);
				}
				len[parse_num - 1] = token_arr_len - (token_arr_len / parse_num * (parse_num - 1));

				for (size_t i = 0; i < parse_num; ++i) {
					_stack[i].push_back(-1);
					thr[i] = std::thread(__LoadData2, token_arr, itemTypeCount, 
						token_arr_start, token_arr_len / parse_num * i, len[i],
						&_stack[i], &_temp[i]);
				}
				...
				for (size_t i = 1; i < parse_num; ++i) {
					size_t idx = 1;
					while (_stack[i].back() == -2) {
						itemTypeCount[_stack[i - 1].back()] += _temp[i][idx];
						_stack[i].pop_back();
						idx++;
					}
				}
...

			{
				...
				if (pivot_num > 0) {
					...
					for (int i = 0; i < pivot_num; ++i) {
						pivot.push_back(FindDivisionPlace(buffer, token_arr + token_arr_start, (num / (pivot_num + 1)) * (i), (num / (pivot_num + 1)) * (i + 1) - 1));
					}
...

				{
					...
					std::vector<int> err(pivots.size() + 1, 0);
					{
						...
						thr[0] = std::thread(__LoadData, buffer, token_arr + token_arr_start, itemTypeCount, _token_arr_len, &__global[0], 0, 0, &next[0], &err[0]);
					}

					for (size_t i = 1; i < pivots.size(); ++i) {
						int64_t _token_arr_len = pivots[i] - (pivots[i - 1] + 1) + 1;

						thr[i] = std::thread(__LoadData, buffer, token_arr + token_arr_start + pivots[i - 1] + 1, itemTypeCount + pivots[i - 1] + 1, _token_arr_len, &__global[i], 0, 0, &next[i], &err[i]);
					...
					if (pivots.size() >= 1) {
						...
						thr[pivots.size()] = std::thread(__LoadData, buffer, token_arr + token_arr_start +  pivots.back() + 1, itemTypeCount + pivots.back() + 1, _token_arr_len, &__global[pivots.size()],
