...

			{
...

				{
					...
					std::vector<int> err(pivots.size() + 1, 0);
					{
						...
						thr[0] = std::thread(__LoadData, buffer, token_arr + token_arr_start, _token_arr_len, &__global[0], 0, 0, &next[0], &err[0]);
					}

					for (size_t i = 1; i < pivots.size(); ++i) {
						int64_t _token_arr_len = pivots[i] - (pivots[i - 1] + 1) + 1;

						thr[i] = std::thread(__LoadData, buffer, token_arr + token_arr_start + pivots[i - 1] + 1, _token_arr_len, &__global[i], 0, 0, &next[i], &err[i]);
					...
					if (pivots.size() >= 1) {
						...
						thr[pivots.size()] = std::thread(__LoadData, buffer, token_arr + token_arr_start +  pivots.back() + 1, _token_arr_len, &__global[pivots.size()],
